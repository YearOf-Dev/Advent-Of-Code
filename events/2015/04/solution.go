...
func part1(input []string) int {
	...
	for {
		test := fmt.Sprintf("%s%d", key, ans)
		data := md5.Sum([]byte(test))
...
// | Part 2
// ----------------------------------------------------------------------------------------------------
func part2(input []string, startAt int) int {
	key := input[0]
	ans := startAt
...
func main() {
	...
	path := aoc_utils.GetFilePath(2015, 04, fileName)
	...
	p2Result := aoc_utils.MeasurePerformance(func() interface{} {
		startPoint, _ := strconv.Atoi(fmt.Sprintf("%x", p1Result.Result))
		return part2(content, startPoint)
	})
